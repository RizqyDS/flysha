    ...
    <div className="bg-white flex flex-col rounded-[20px] w-[340px]">
      <div className="flex flex-col p-[20px_20px_25px] border-b-2 border-dotted border-flysha-grey gap-4 relative">
        ...
        <div className="flex justify-between items-center">
          <div className="flex flex-col gap-[2px]">
            <p className="font-bold text-lg text-flysha-black">{data.flight.plane.name}</p>
            <p className="text-sm text-flysha-grey">{data.flight.plane.code} • {data.seat.type} Class</p>
          </div>
      ...
      <div className="flex flex-col gap-[10px] p-[25px_20px_20px]">
        <div className="flex justify-between text-flysha-black">
          <span>Date</span>
          <span className="font-semibold">{dateFormat(data.bookingDate, "DD MM YYYY")}</span>
        </div>
        <div className="flex justify-between text-flysha-black">
          <span>Time</span>
          <span className="font-semibold">{dateFormat(data.flight.departureDate, "HH:mm")} - {dateFormat(data.flight.arrivalDate, "HH:mm")}</span>
        </div>
        <div className="flex justify-between text-flysha-black">
          <span>Airport</span>
          <span className="font-semibold">{data.flight.departureCityCode} - {data.flight.destinationCityCode}</span>
        </div>
        <div className="flex justify-between text-flysha-black">
          <span>Name</span>
