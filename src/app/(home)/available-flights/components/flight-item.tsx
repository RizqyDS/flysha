...
import { type FContext, FlightContext, type FlightWithPlane } from "../providers/flight-provider";
import { getUrlFile } from "@/lib/supabase";
import { CHECKOUT_KEY, SEAT_VALUES, SeatValuesType, dateFormat, rupiahFormat } from "@/lib/utils";
      ...
      <div className="ticket-card flex justify-between items-center rounded-[20px] p-5 bg-flysha-bg-purple">
        <div className="flex gap-[16px] items-center">
          ...
          <div className="flex flex-col justify-center-center gap-[2px]">
            ...
            <p className="text-sm text-flysha-off-purple">{selectedSeat.label}{""} Class</p>
          </div>
        </div>
        ...
        <p className="w-fit h-fit font-bold text-lg">{rupiahFormat(data.price + selectedSeat.additionalPrice)}</p>
        <button
          type="button"
          onClick={bookNow}
